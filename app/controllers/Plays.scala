...
class Plays @Inject() (json4s: Json4s) extends InjectedController with Authenticator {
  import Responses._
  import json4s._
  ...
  def board() = Action(json) { implicit req =>
    ...
    withAuth(NormalUser) { user =>
      DB localTx { implicit session =>
        val result: Either[Result, Result] = for {
          ...
          game <- Game.findBy(sqls.eq(g.accountId, user.id).and.eq(g.missionId, b.missionId))
  ...
  def undo(missionId: Long) = Action { implicit req =>
    import DefaultAliases.{g, gh}
    withAuth(NormalUser) { user =>
      Game.findBy(sqls.eq(g.accountId, user.id).and.eq(g.missionId, missionId)).fold(notFound("mission")) { game =>
  ...
  def clear(missionId: Long) = Action(json) { implicit req =>
    import DefaultAliases.{g, gp}
    withAuth(NormalUser) { user =>
      DB localTx { implicit session =>
        val result = for {
          ...
          game <- Game.findBy(sqls.eq(g.accountId, user.id).and.eq(g.missionId, missionId))
            .toRight(notFound("Mission"))
